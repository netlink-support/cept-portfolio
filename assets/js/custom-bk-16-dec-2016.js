$(document).ready(function () {
    ...
    $("#filer_input2").filer({
							...
							<div class="jFiler-item-inner">\
								...
								<div class="jFiler-item-text">\
									<textarea placeholder="Image caption" name="caption_{{fi-intval}}" id="caption_{{fi-intval}}" rows="2" width="100%">{{fi-caption}}</textarea>\
                                                                        {{fi-groupwork}}&nbsp;<label for="group_work_{{fi-intval}}">Part of group excercise</label><br/>\
                                                                        {{fi-reftextbox}}\
        ...
        onRemove: function (itemEl, file, id, listEl, boxEl, newInputEl, inputEl) {
            ...
            $.post(js_base + "remove-visual", {file: file_name, course_code: course_code_js});
            //total_visuals--;
            //$('#total_visuals').val(total_visuals);
        },
        onEmpty: null,
...
function img() {
    ...
    $.each(inp, function (key, value) {
        inp = '{"name": "' + value + '","size": 5453,"type": "image/jpg","file": "assets/projects/visuals/2016/' + value + '"}';
        imag = $.parseJSON(inp);
        images.push(imag);
    });
...
function changePosition(new_pos, old_pos)
{
