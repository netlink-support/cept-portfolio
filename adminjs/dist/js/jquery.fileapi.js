...
(function ($, api) {
    ...
    Plugin.prototype = {
        ...
        _extractDuplicateFiles: function (list/**
             * Array
             */
) {
        ...
        _getFormatedSize: function (size) {
            ...
            return opts.sizeFormat.replace(
                /^\d+([^\d]+)(\d*)/, function (_, separator, fix) {
        ...
        _listen: function (name, selector, fn) {
            selector && _each(
                $.trim(selector).split(','), function (selector) {
        ...
        serialize: function () {
            ...
            this.$el.find(':input').each(
                function (name, node) {
                    if((name = node.name) && !node.disabled
        ...
        upload: function () {
            if(!this.active && this.emit('beforeUpload', { widget: this, files: this.queue }) ) {
                ...
                // Add event listeners
                _each(
                    ['Upload', 'Progress', 'Complete'], function (name) {
                        ...
                        this, function (err, info) {
                            if(err ) {
                                ...
                                if(this.__cropFile !== file ) {
                                    ...
                                    api.Image(file).rotate(opts.imageAutoOrientation ? 'auto' : 0).get(
                                        function (err, img) {
        ...
        remove: function (file) {
            ...
            this.queue = api.filter(
                this.queue, function (file) {
                    return api.uid(file) != uid; }
            );
            this.files = api.filter(
                this.files, function (file) {
                    return api.uid(file) != uid; }
            );
        ...
        destroy: function () {
            ...
            _each(
                this.options.elements.ctrl, function (selector) {
    ...
    function _extractFilesOverLimit(limit, countFiles, files, other)
    {
        if(limit ) {
            ...
            if(delta > 0 ) {
                _each(
                    files.splice(0, delta), function (file, i) {
    ...
    $.fn.fileapi.version = '0.4.3';
    $.fn.fileapi.tpl = function (text) {
        var index = 0;
        var source = "__b+='";

        text.replace(
            /(?:&lt;|<)%([-=])?([\s\S]+?)%(?:&gt;|>)|$/g, function (match, mode, expr, offset) {
        ...
        if(typeof opts === 'string' ) {
            ...
            api.getInfo(
                file, function (err, info) {
                    ...
                    Image.rotate(deg === void 0 ? 'auto' : deg).get(
                        function (err, img) {
                            var
                            ...
                            _each(
                                ['onSelect', 'onChange'], function (name, fn) {
                                    if(fn = opts[name] ) {
