...
QUnit.test(
    "Filtering a zip", function (assert) {
        var zip = new JSZip();
        zip.file("1.txt", "1\n");
        zip.file("2.txt", "2\n");
        zip.file("3.log", "3\n");
        var result = zip.filter(
...
QUnit.test(
    "Filtering a zip from a relative path", function (assert) {
        var zip = new JSZip();
        ...
        var count = 0;
        var result = zip.folder("foo").filter(
            function (relativeFilename) {
                count++;
...
QUnit.test(
    "Filtering a zip : the full path is still accessible", function (assert) {
        var zip = new JSZip();
        zip.file("foo/1.txt", "1\n");
        zip.file("foo/2.txt", "2\n");
        zip.file("foo/3.log", "3\n");
