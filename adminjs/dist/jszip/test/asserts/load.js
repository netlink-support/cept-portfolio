...
QUnit.module(
    "load", function () {
        ...
        JSZipTestUtils.testZipFile(
            "load(string) works", "ref/all.zip", function (assert, file) {
        ...
        JSZipTestUtils.testZipFile(
            "Load files which shadow Object prototype methods", "ref/pollution.zip", function (assert, file) {
                var done = assert.async();
                assert.ok(typeof file === "string");
                JSZip.loadAsync(file)
                .then(
        ...
        JSZipTestUtils.testZipFile(
            "load(string) handles bytes > 255", "ref/all.zip", function (assert, file) {
                var done = assert.async();
        ...
        JSZipTestUtils.testZipFile(
            "load(Array) works", "ref/deflate.zip", function (assert, file) {
                var done = assert.async();
        ...
        JSZipTestUtils.testZipFile(
            "load(array) handles bytes > 255", "ref/deflate.zip", function (assert, file) {
                var done = assert.async();
                var updatedFile = new Array(file.length);
                for( var i = 0; i < file.length; ++i ) {
        ...
        if (JSZip.support.arraybuffer) {
            JSZipTestUtils.testZipFile(
                "load(ArrayBuffer) works", "ref/all.zip", function (assert, fileAsString) {
        ...
        if (JSZip.support.nodebuffer) {
            JSZipTestUtils.testZipFile(
                "load(Buffer) works", "ref/all.zip", function (assert, fileAsString) {
                    var done = assert.async();
        ...
        if (JSZip.support.uint8array) {
            JSZipTestUtils.testZipFile(
                "load(Uint8Array) works", "ref/all.zip", function (assert, fileAsString) {
                    var done = assert.async(3);
        ...
        JSZipTestUtils.testZipFile(
            "zip with DEFLATE", "ref/deflate.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "read zip with comment", "ref/archive_comment.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "generate zip with comment", "ref/archive_comment.zip", function (assert, file) {
                var zip = new JSZip();
                zip.file("Hello.txt", "Hello World\n", {comment:"entry comment"});
                var done = assert.async();
                zip.generateAsync({type:"binarystring", comment:"file comment"}).then(
                    function (generated) {
        ...
        JSZipTestUtils.testZipFile(
            "zip with extra attributes", "ref/extra_attributes.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "zip 64", "ref/zip64.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "zip with data descriptor", "ref/data_descriptor.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "nested zip", "ref/nested.zip", function (assert, file) {
        ...
        JSZipTestUtils.testZipFile(
            "nested zip with data descriptors", "ref/nested_data_descriptor.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "nested zip 64", "ref/nested_zip64.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "Zip text file with UTF-8 characters in filename", "ref/utf8_in_name.zip", function (assert, file) {
                var done = assert.async(2);
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "Zip text file with UTF-8 characters in filename and windows compatibility", "ref/winrar_utf8_in_name.zip", function (assert, file) {
                var done = assert.async(2);
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "Zip text file with backslash in filename", "ref/backslash.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "Zip text file from windows with \\ in central dir", "ref/slashes_and_izarc.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "zip file with prepended bytes", "ref/all_prepended_bytes.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
        ...
        JSZipTestUtils.testZipFile(
            "zip file with appended bytes", "ref/all_appended_bytes.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function success(zip)
        ...
        JSZipTestUtils.testZipFile(
            "zip64 file with extra bytes", "ref/zip64_prepended_bytes.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function success(zip)
        ...
        JSZipTestUtils.testZipFile(
            "zip64 file with extra bytes", "ref/zip64_appended_bytes.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function success(zip)
        ...
        JSZipTestUtils.testZipFile(
            "load(promise) works", "ref/all.zip", function (assert, fileAsString) {
                var done = assert.async();
        ...
        if (JSZip.support.blob) {
            JSZipTestUtils.testZipFile(
                "load(blob) works", "ref/all.zip", function (assert, fileAsString) {
        ...
        JSZipTestUtils.testZipFile(
            "valid crc32", "ref/all.zip", function (assert, file) {
                var done = assert.async();
        ...
        JSZipTestUtils.testZipFile(
            "loading in a sub folder", "ref/all.zip", function (assert, file) {
                var done = assert.async();
                var zip = new JSZip();
        ...
        JSZipTestUtils.testZipFile(
            "loading overwrite files", "ref/all.zip", function (assert, file) {
                var done = assert.async();
                var zip = new JSZip();
        ...
        JSZipTestUtils.testZipFile(
            "basic encryption", "ref/encrypted.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function success()
        ...
        JSZipTestUtils.testZipFile(
            "bad compression method", "ref/invalid/compression.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function success()
        ...
        JSZipTestUtils.testZipFile(
            "zip file with missing bytes", "ref/all_missing_bytes.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function success()
        ...
        JSZipTestUtils.testZipFile(
            "zip64 file with missing bytes", "ref/zip64_missing_bytes.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function success()
        ...
        JSZipTestUtils.testZipFile(
            "zip file with extra field is Non-standard", "ref/extra_filed_non_standard.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function success()
        ...
        QUnit.test(
            "not a zip file", function (assert) {
                var done = assert.async();
        ...
        QUnit.test(
            "truncated zip file", function (assert) {
                var done = assert.async();
        ...
        JSZipTestUtils.testZipFile(
            "invalid crc32 but no check", "ref/invalid/crc32.zip", function (assert, file) {
                var done = assert.async();
        ...
        JSZipTestUtils.testZipFile(
            "invalid crc32", "ref/invalid/crc32.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file, {checkCRC32:true})
                .then(
                    function success()
        ...
        JSZipTestUtils.testZipFile(
            "bad offset", "ref/invalid/bad_offset.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file, {checkCRC32:false})
                .then(
                    function success()
        ...
        JSZipTestUtils.testZipFile(
            "bad decompressed size, read a file", "ref/invalid/bad_decompressed_size.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        JSZipTestUtils.testZipFile(
            "bad decompressed size, generate a zip", "ref/invalid/bad_decompressed_size.zip", function (assert, file) {
                var done = assert.async();
                JSZip.loadAsync(file)
                .then(
                    function (zip) {
        ...
        if (typeof window === "undefined" || QUnit.urlParams.complexfiles) {
            ...
            JSZipTestUtils.testZipFile(
                "Franz Kafka - The Metamorphosis.epub", "ref/complex_files/Franz Kafka - The Metamorphosis.epub", function (assert, file) {
                    var done = assert.async(2);
                    JSZip.loadAsync(file)
                    .then(
                        function (zip) {
                            assert.equal(
                                zip.filter(
                                    function () {
                                        return true;}
                                ).length, 26, "the zip contains the good number of elements."
                            );
            ...
            JSZipTestUtils.testZipFile(
                "Outlook2007_Calendar.xps, createFolders: false", "ref/complex_files/Outlook2007_Calendar.xps", function (assert, file) {
                        ...
                        function (zip) {
                            ...
                            assert.equal(
                                zip.filter(
                                    function () {
                                        return true;}
                                ).length, 15, "the zip contains the good number of elements."
                            );
            ...
            JSZipTestUtils.testZipFile(
                "Outlook2007_Calendar.xps, createFolders: true", "ref/complex_files/Outlook2007_Calendar.xps", function (assert, file) {
                    var done = assert.async();
                        ...
                        function (zip) {
                            ...
                            assert.equal(
                                zip.filter(
                                    function () {
                                        return true;}
                                ).length, 23, "the zip contains the good number of elements."
                            );
                            return zip.file("[Content_Types].xml").async("string");
                        }
                    )
            ...
            JSZipTestUtils.testZipFile(
                "AntarcticaTemps.xlsx, createFolders: false", "ref/complex_files/AntarcticaTemps.xlsx", function (assert, file) {
                    var done = assert.async();
                    JSZip.loadAsync(file, {createFolders: false})
                    .then(
                        function (zip) {
                            ...
                            assert.equal(
                                zip.filter(
                                    function () {
                                        return true;}
                                ).length, 17, "the zip contains the good number of elements."
                            );
                            return zip.file("[Content_Types].xml").async("string");
                        }
                    ).then(
            ...
            // Same test as above, but with createFolders option set to true
            JSZipTestUtils.testZipFile(
                "AntarcticaTemps.xlsx, createFolders: true", "ref/complex_files/AntarcticaTemps.xlsx", function (assert, file) {
                    var done = assert.async();
                    JSZip.loadAsync(file, {createFolders: true})
                    .then(
                        function (zip) {
                            ...
                            assert.equal(
                                zip.filter(
                                    function () {
                                        return true;}
                                ).length, 27, "the zip contains the good number of elements."
                            );
                            return zip.file("[Content_Types].xml").async("string");
                        }
                    ).then(
            ...
            JSZipTestUtils.testZipFile(
                "AntarcticaTemps.ods, createFolders: false", "ref/complex_files/AntarcticaTemps.ods", function (assert, file) {
                    var done = assert.async();
                    JSZip.loadAsync(file, {createFolders: false})
                    .then(
                        function (zip) {
                            ...
                            assert.equal(
                                zip.filter(
                                    function () {
                                        return true;}
                                ).length, 20, "the zip contains the good number of elements."
                            );
            ...
            JSZipTestUtils.testZipFile(
                "AntarcticaTemps.ods, createFolders: true", "ref/complex_files/AntarcticaTemps.ods", function (assert, file) {
                    var done = assert.async();
                    JSZip.loadAsync(file, {createFolders: true})
                    .then(
                        function (zip) {
