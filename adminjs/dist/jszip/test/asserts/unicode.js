...
JSZipTestUtils.testZipFile(
    "Zip text file with UTF-8 characters", "ref/utf8.zip", function (assert, expected) {
        var zip = new JSZip();
...
QUnit.test(
    "Text file with long unicode string", function (assert) {
        var expected = "€";
        for(var i = 0; i < 13; i++) {
...
JSZipTestUtils.testZipFile(
    "Zip text file with UTF-8 characters in filename", "ref/utf8_in_name.zip", function (assert) {
        var zip = new JSZip();
        zip.file("€15.txt", "€15\n");
        var done = assert.async();
        zip.generateAsync({type:"binarystring"}).then(
...
JSZipTestUtils.testZipFile(
    "Zip text file with non unicode characters in filename: loadAsync without decodeFileName", "ref/local_encoding_in_name.zip", function (assert, content) {

        var done = assert.async();
        JSZip.loadAsync(content).then(
            function (zipUnicode) {
...
JSZipTestUtils.testZipFile(
    "Zip text file with non unicode characters in filename: loadAsync with decodeFileName", "ref/local_encoding_in_name.zip", function (assert, content) {
        var conversions = {
...
JSZipTestUtils.testZipFile(
    "Zip text file with non unicode characters in filename: generateAsync with encodeassert, fileName", "ref/local_encoding_in_name.zip", function (assert, content) {
        var conversions = {
            "": [],
...
JSZipTestUtils.testZipFile(
    "Zip text file and UTF-8, Pile Of Poo test", "ref/pile_of_poo.zip", function (assert, expected) {
        var zip = new JSZip();
        ...
        zip.generateAsync({type:"binarystring", comment : text}).then(
            function (actual) {
