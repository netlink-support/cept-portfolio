...
testGenerateFor(
    [{
        ...
        JSZipTestUtils.testZipFile(
            "generate : type:string. " + testName, file, function (assert, expected) {
                testGenerate(
                    assert, {
        ...
        JSZipTestUtils.testZipFile(
            "generate : type:base64. " + testName, file, function (assert) {
                testGenerate(
                    assert, {
                        prepare : function () {
        ...
        JSZipTestUtils.testZipFile(
            "generate : type:uint8array. " + testName, file, function (assert, expected) {
                testGenerate(
                    assert, {
                        prepare : JSZipTestUtils.createZipAll,
        ...
        JSZipTestUtils.testZipFile(
            "generate : type:arraybuffer. " + testName, file, function (assert, expected) {
                testGenerate(
                    assert, {
                        prepare : JSZipTestUtils.createZipAll,
        ...
        JSZipTestUtils.testZipFile(
            "generate : type:nodebuffer. " + testName, file, function (assert, expected) {
                testGenerate(
                    assert, {
                        prepare : JSZipTestUtils.createZipAll,
        ...
        JSZipTestUtils.testZipFile(
            "generate : type:blob. " + testName, file, function (assert, expected) {
                testGenerate(
                    assert, {
                        prepare : JSZipTestUtils.createZipAll,
        ...
        JSZipTestUtils.testZipFile(
            "generate : type:blob mimeType:application/ods. " + testName, file, function (assert, expected) {
                testGenerate(
                    assert, {
                        prepare : JSZipTestUtils.createZipAll,
...
testGenerateFor(
    [{
        ...
        JSZipTestUtils.testZipFile(
            "STORE doesn't compress, " + testName, file, function (assert, expected) {
                testGenerate(
                    assert, {
                        prepare : function () {
                            var zip = new JSZip();
...
testGenerateFor(
    [{
        ...
        JSZipTestUtils.testZipFile(
            "DEFLATE compress, " + testName, file, function (assert, expected) {
                testGenerate(
                    assert, {
                        prepare : function () {
                            var zip = new JSZip();
...
JSZipTestUtils.testZipFile(
    "STORE is the default method", "ref/text.zip", function (assert, expected) {
        var zip = new JSZip();
        zip.file("Hello.txt", "Hello World\n");
        var done = assert.async();
        zip.generateAsync({type:"binarystring", compression:"STORE"}).then(
            function (content) {
...
{
    var done = assert.async();
    JSZipTestUtils.createZipAll().generateAsync({type:"binarystring", compression:from}).then(
        function (actual) {
            done();
            testGenerate(
                assert, {
...
QUnit.test(
    "Lazy decompression works", function (assert) {
...
JSZipTestUtils.testZipFile(
    "empty zip", "ref/empty.zip", function (assert, expected) {
        testGenerate(
            assert, {
                prepare : function () {
                    var zip = new JSZip();
...
QUnit.test(
    "unknown compression throws an exception", function (assert) {
        testGenerate(
            assert, {
                prepare : JSZipTestUtils.createZipAll,
...
QUnit.test(
    "missing type throws an exception", function (assert) {
        testGenerate(
            assert, {
                prepare : JSZipTestUtils.createZipAll,
                options : {},
...
QUnit.test(
    "generateAsync uses the current folder level", function (assert) {
        var done = assert.async();
...
QUnit.test(
    "generateAsync keep the explicit / folder", function (assert) {
        var done = assert.async();
...
JSZipTestUtils.testZipFile(
    "generate with promises as files", "ref/all.zip", function (assert, expected) {
        var done = assert.async();
        var zip = new JSZip();
        zip.file(
            "Hello.txt", new JSZip.external.Promise(
                function (resolve) {
                        setTimeout(
                            function () {
                                resolve("Hello World\n");
        ...
        zip.folder("images").file(
            "smile.gif", new JSZip.external.Promise(
                function (resolve) {
                        setTimeout(
                            function () {
