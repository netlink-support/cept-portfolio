...
QUnit.module(
    "stream", function () {
        ...
        QUnit.test(
            "A stream is pausable", function (assert) {
        ...
        function generateStreamTest(name, ref, createFunction, generateOptions)
        {
            JSZipTestUtils.testZipFile(
                name,ref, function (assert, expected) {
                    var done = assert.async();
        ...
        if (JSZip.support.nodestream) {
            ...
            zipObjectStreamTest(
                "ZipObject#nodeStream generates a working stream[nodebuffer]", function () {
                    var zip = JSZipTestUtils.createZipAll();
                    return zip.file("Hello.txt").nodeStream("nodebuffer");
                }
            );
            zipObjectStreamTest(
                "ZipObject#nodeStream generates a working stream[default]", function () {
                    var zip = JSZipTestUtils.createZipAll();
            ...
            QUnit.test(
                "a ZipObject containing a stream can be read with async", function (assert) {
                    var done = assert.async();
                    var stream = JSZipTestUtils.createZipAll().file("Hello.txt").nodeStream();
                    var zip = new JSZip();
                    zip.file("Hello.txt", stream);
                    zip.file("Hello.txt").async("text").then(
                        function (actual) {
            ...
            QUnit.test(
                "a ZipObject containing a stream can't be read with async 2 times", function (assert) {
                    var done = assert.async();
            ...
            QUnit.test(
                "a ZipObject containing a stream can't be read with nodeStream 2 times", function (assert) {
                    var done = assert.async();
            ...
            QUnit.test(
                "generateAsync with a stream can't be read 2 times", function (assert) {
                    var done = assert.async();
            ...
            QUnit.test(
                "generateNodeStream with a stream can't be read 2 times", function (assert) {
                    var done = assert.async();
            ...
            QUnit.test(
                "loadAsync ends with an error when called with a stream", function (assert) {
                    var done = assert.async();
                    var stream = JSZipTestUtils.createZipAll().generateNodeStream({"type":"nodebuffer"});
                    JSZip.loadAsync(stream).then(
                        function () {
            ...
            QUnit.test(
                "generateNodeStream generates an error", function (assert) {
                    try {
                        var zip = new JSZip();
            ...
            QUnit.test(
                "ZipObject#nodeStream generates an error", function (assert) {
                    try {
                        var zip = JSZipTestUtils.createZipAll();
