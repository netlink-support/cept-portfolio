...
QUnit.test(
    "JSZip.external.Promise", function (assert) {
...
QUnit.test(
    "load JSZip doesn't override the global Promise", function (assert) {
...
QUnit.test(
    "external.Promise can be replaced in .async()", function (assert) {
        ...
        var promise = JSZipTestUtils.createZipAll().file("Hello.txt").async("string").then(
            function () {
...
QUnit.test(
    "external.Promise can be replaced in .generateAsync()", function (assert) {
        var done = assert.async();
        var OriginalPromise = JSZip.external.Promise;
        var MyShinyPromise = createPromiseProxy(OriginalPromise);
        ...
        var promise = JSZipTestUtils.createZipAll().generateAsync({type:"string"}).then(
            function () {
                assert.ok(MyShinyPromise.calls > 0, "at least 1 call of the new Promise");
                JSZip.external.Promise = OriginalPromise;
                done();
...
JSZipTestUtils.testZipFile(
    "external.Promise can be replaced in .loadAsync()", "ref/all.zip", function (assert, all) {
        var done = assert.async();
        var OriginalPromise = JSZip.external.Promise;
        var MyShinyPromise = createPromiseProxy(OriginalPromise);
        ...
        var promise = JSZip.loadAsync(all).then(
            function () {
                assert.ok(MyShinyPromise.calls > 0, "at least 1 call of the new Promise");
                JSZip.external.Promise = OriginalPromise;
                done();
